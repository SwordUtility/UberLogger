...
namespace HsufAOT.Editor
{
    ...
    public class UberLoggerEditorWindow : EditorWindow, UberLoggerEditor.ILoggerWindow
    {
        ...
        void OnEnable()
        {
            ...
            titleContent.text = "Hsuf Console";
#else
        title = "Hsuf Console";
        ...
        void AddLogInfoToRenderQueue()
        {
            if (EditorLogger.GetLogInfo().Count != CurrentLogList.Count)
            {
                for (int i = CurrentLogList.Count; i < EditorLogger.GetLogInfo().Count; i++)
                {
                    var log = EditorLogger.GetLogInfo()[i];
                    CurrentLogList.Add(log);
                    if (Collapse)
                    {
                        if (ShouldShowLog(m_tFilterRegex, log))
                        {
                            ...
                            var logLineSize = EntryStyleBackEven.CalcSize(content);
        ...
        public void DrawLogList(float height)
        {
            ...
            if (ScrollFollowMessages)
            {
                ...
                {
                    // UnityEngine.Debug.Log(String.Format("{0} {1}", lastScrollPosition.y, LogListScrollPosition.y));
            ...
            for (int renderLogIndex = firstRenderLogIndex; renderLogIndex < lastRenderLogIndex; renderLogIndex++)
            {
                ...
                {
                    GUI.backgroundColor = new Color(0.2f, 0.5f, 1);
                }
                else
                {
        ...
        void DrawFilter()
        {
            ...
            if (filterRegex != FilterRegex)
            {
                ...
                {
                    m_tFilterRegex = new Regex(FilterRegex);
                }
                else
                {
                    m_tFilterRegex = null;
        ...
        System.Text.RegularExpressions.Regex m_tFilterRegex = null;
        string FilterRegex = null;
